...
from letsql.expr.relations import into_backend as into_backend_fun
...
def bridge(expr: ir.Expr) -> ir.Expr:
    from functools import partial

    from letsql import to_pyarrow, to_pyarrow_batches

    if hasattr(expr, "into_backend"):
        return expr

    for attr in dir(expr):
        if not attr.startswith("_"):
            fun = getattr(expr, attr)
            if callable(fun) and attr not in ("op", "schema"):
                object.__setattr__(expr, attr, wrap_with_bridge_expr(fun))

    object.__setattr__(expr, "execute", partial(execute, expr))
    object.__setattr__(expr, "to_pyarrow_batches", partial(to_pyarrow_batches, expr))
    object.__setattr__(expr, "to_pyarrow", partial(to_pyarrow, expr))
    object.__setattr__(expr, "into_backend", partial(into_backend, expr))
    object.__setattr__(expr, "cache", partial(cache, expr))
    object.__setattr__(expr, "__repr__", partial(plan_str, expr))
...
def into_backend(self, backend, name=None):
    new_ibis_expr = into_backend_fun(self, backend, name)
    return bridge(new_ibis_expr)


def cache(self, storage=None):
    try:
    ...
    new_expr = maybe_prevent_cross_source_caching(self, storage)
...
#         try:
#             attr = getattr(self._ibis_expr, item)
#         except Exception:
#             raise AttributeError
#
#         if callable(attr):
#             attr = wrap_with_bridge_expr(attr)
#
#         return attr
